...
nav_order: 2
lang: ja
hreflang_alt: "en/usage_cli"
hreflang_lang: "en"
---

{: .warning }
> **この日本語ページは準備中です。** 技術的な詳細は英語版が最新かつ正確です。 → [English version]({{ '/en/usage_cli/' | relative_url }})
...
`--functional` は `svwn5`（LDA）と `pbe`（GGA）、`--grid_level` は 0–9 で PySCF のグリッドレベルと同じ意味です。閉殻の RKS のみ対応。詳細は [DFT parameters](parameters.html#dft-parametersdft-パラメータ)。
