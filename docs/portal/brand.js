...
(function () {
  ...
  // GANSU2 にはまだ販売パートナーがいないので空。
  // （移植元の QUBO++ には Hi-QUBO のテーマが入っていたが、別製品なので除去した。
  //   富士通との取り決めが決まったらここにエントリを足す。）
  const BRANDS = [];
  ...
  const STORAGE_KEY = "gansu_portal_brand";
  ...
  if (brand) {
    ...
    // Came from our own documentation site: that is the GANSU2 front door,
  ...
  function applyTheme() {
    ...
    if (entry.icons && !document.querySelector("link[data-brand-icon]")) {
      ...
      // /favicon.ico (the GANSU2 one); explicit links take precedence over it.
  ...
  window.GANSU_BRAND = brand || PRODUCT;
