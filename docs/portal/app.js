...
(function () {
  ...
  async function saveCredential(email, password) {
    if (!window.PasswordCredential || !navigator.credentials || !navigator.credentials.store) {
      console.warn("[GANSU2] PasswordCredential API not available; password not saved.");
      return;
    }
    if (!email || !password) {
      console.warn("[GANSU2] saveCredential: missing email or password, skipping.");
      return;
    }
    try {
      ...
      console.log("[GANSU2] Credential stored:", stored ? stored.id : "(null result — Chrome may show key icon in address bar instead of a popup)");
    } catch (e) {
      console.warn("[GANSU2] saveCredential failed:", e);
    }
  }
  ...
  function renderLicense(lic, acts, actsError) {
    ...
    if (isPortalTrial) {
      ...
      if (allowed) {
        ...
        renewBtn = `<button type="button" class="renew-ready" data-renew="${escapeHTML(lic.license_key)}" title="Start or extend this license's expiry date (keeps the same license key)">Start/Renew</button>`;
        renewBadge = ` <span class="renew-badge">Start/Renewal available</span>`;
      } else {
        const dt = fmtDateTime(avail);
        renewBtn = `<button type="button" class="secondary" data-renew="${escapeHTML(lic.license_key)}" disabled title="Start/Renewal available from ${escapeHTML(dt)}">Start/Renew</button>`;
      }
    }
